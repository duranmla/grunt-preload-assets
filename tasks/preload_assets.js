...
module.exports = function ( grunt ) {
	...
	var path = require( 'path' ),
		...
		JSONminify = JSONminify || require( "node-json-minify" );

	/**
	 * VINOGA - Definitely not stable for now. Just comment it out.
	 */
	// Image libs
	...
	grunt.registerMultiTask( 'preload_assets', 'A Grunt plugin for making preload assets manifest.', function () {
		...
		/**
		 * VINOGA - Use a new `template` var so we don't override the option.
		 */
		var template;
		...
		// TODO: USE GRUNT
		var tpl = options.processSrc( grunt.file.read( template ) );
		...
		this.files.forEach( function ( file ) {
			...
			var src = file.src.filter( function ( filepath ) {
				...
				// Add file src
				var fileProps = {};
				fileProps.src = options.srcBasePath + getPathWithoutBasePath( filepath, options.ignoreBasePath );
			...
			try {
				...
				/**
				 * VINOGA - Minify JSON output if specified.
				 */
				if ( options.template === 'json' && options.minified ) {
					grunt.verbose.writeln( "Minifiying file..." );
					compiled = JSONminify( compiled );
				}
			} catch ( e ) {
				grunt.log.error( e );
